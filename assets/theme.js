...
theme.ShopifyVideo = (function() {
  var selectors = {
    productMediaWraps: '.product-single__media',
    videoWrapper: '.shopify-video-wrapper',
    videoPlayer: '.video-player',
    controls: '.video-controls',
    pauseOverlay: '.pause-overlay',
    playButton: '.play-pause',
    playIcon: '.play-pause .play-icon',
    pauseIcon: '.play-pause .pause-icon',
    muteIcon: '.mute .mute-icon',
    unmuteIcon: '.mute .unmute-icon',
    muteButton: '.mute',
    volumeWrapper: '.volume-wrapper',
    volumeBar: '.volume-bar',
    seekBar: '.seek-bar',
    flashOverlay: '.flash-wrapper'
  };
  ...
  var hideVolumeTimer;
  var flashTiming = 500;
  var shopifyVideoLoaded = false;

  function setActiveVideo(playerId) {
    $activeVideo = videos[playerId];
  }

  function show(object) {
    object.addClass('visible-controls');
    object.removeClass('hidden-controls');
  }

  function hide(object) {
    object.removeClass('visible-controls');
    object.addClass('hidden-controls');
  }

  function touchEvent(originalEvent) {
  ...
  function rangeTouchPercent(evt, vertical) {
    var $target = $(evt.currentTarget);
    var touch = touchEvent(evt.originalEvent);
    ...
    var position;
    if (vertical) {
      position = $target.offset().top - touch.pageY + width;
    } else {
      position = touch.pageX - $target.offset().left;
    }
  ...
  function flash(icon, playerId) {
    var $video = videos[playerId] || $activeVideo;
    if (!icon || $video.controlsVisible || $video.pauseOverlay) {
      return;
    }
    flashShow(icon);
    setTimeout(function() {
  ...
  function flashShow(icon, playerId) {
    var $video = videos[playerId] || $activeVideo;
    $video.$flashOverlay.find('.icon').addClass('hide');
    $video.$flashOverlay.find('.icon-' + icon).removeClass('hide');
    $video.flashVisible = true;
    show($video.$flashOverlay);
  }

  function flashHide(icon, playerId) {
    var $video = videos[playerId] || $activeVideo;
    hide($video.$flashOverlay);
    $video.$flashOverlay.find('.icon-' + icon).addClass('hide');
    $video.flashVisible = false;
  }

  function showPauseOverlay(playerId) {
    var $video = videos[playerId] || $activeVideo;
    if (!$video.player.paused || $video.controlsVisible) {
      return;
    }
    if ($video.flashVisible) {
      setTimeout(function() {
        showPauseOverlay(playerId);
      }, flashTiming);
    } else {
      $video.$videoWrapper.find(selectors.pauseOverlay).removeClass('hide');
      $video.pauseOverlay = true;
    }
  }

  function hidePauseOverlay(playerId) {
    var $video = videos[playerId] || $activeVideo;
    $video.$videoWrapper.find(selectors.pauseOverlay).addClass('hide');
    $video.pauseOverlay = false;
  }
  ...
  function playBackFlash(playerId) {
    var $video = videos[playerId] || $activeVideo;
    if ($video.player.paused) {
      flash('play');
    } else {
  ...
  function showControls(playerId) {
    var $video = videos[playerId] || $activeVideo;
    ...
    if ($video.player.paused !== true) {
      show($video.$controls);
      $video.controlsVisible = true;
    }
  }

  function hideControls(playerId) {
    var $video = videos[playerId] || $activeVideo;
    if (!$video.controlsVisible) {
      return;
    }
    hide($video.$controls);
    $video.controlsVisible = false;
    hideVolumeControl();
    showPauseOverlay();
  }
  ...
  function toggleFullscreen(playerId) {
    ...
    } else if ($video.player.mozRequestFullScreen) {
      $video.player.mozRequestFullScreen(); // Firefox
    } else if ($video.player.webkitRequestFullscreen) {
      $video.player.webkitRequestFullscreen(); // Chrome and Safari
    }
  }
  ...
  function setTime(percent, playerId) {
    var $video = videos[playerId] || $activeVideo;
    var value = $video.$controls.find(selectors.seekBar)[0].value;
    percent = percent || value / 100;
    var time = $video.player.duration * percent;
    $video.player.currentTime = time;
    updateSeekBar(playerId);
  }

  function seekVideo(delta, playerId) {
    var $video = videos[playerId] || $activeVideo;
    $video.player.currentTime += delta;
    updateSeekBar(playerId);
  }

  function updateSeekBar(playerId) {
    var $video = videos[playerId] || $activeVideo;
    var percent = (100.0 / $video.player.duration) * $video.player.currentTime;
    updateSeekGradient(percent);
    updateSeekPosition(percent);
  }

  function updateSeekPosition(percent, playerId) {
    var $video = videos[playerId] || $activeVideo;
    $video.$controls.find(selectors.seekBar)[0].value = percent;
  }

  function updateSeekGradient(percent, playerId) {
    var $video = videos[playerId] || $activeVideo;
    ...
    var style =
      ...
      percent +
      '%, #A0A0A0));';
    $video.$controls.find(selectors.seekBar)[0].style = style;
  }

  function resetVolumeWrapperTimeout(timeout) {
    timeout = timeout || 250;
    hideVolumeTimer = setTimeout(function() {
      hideVolumeControl();
    }, timeout);
  }

  function showVolumeControl(playerId) {
    var $video = videos[playerId] || $activeVideo;
    show($video.$volumeWrapper);
  }

  function hideVolumeControl(playerId) {
    var $video = videos[playerId] || $activeVideo;
    hide($video.$volumeWrapper);
  }

  function toggleMute(playerId) {
    var $video = videos[playerId] || $activeVideo;
    if ($video.player.muted === false) {
      mutePlayer();
    } else {
      unmutePlayer();
    }
  ...
  function unmutePlayer(playerId) {
    ...
    setVolumePosition($video.player.volume, playerId);
    $video.$controls.find(selectors.muteIcon).addClass('hide');
    $video.$controls.find(selectors.unmuteIcon).removeClass('hide');
    flash('unmute');
  }
  ...
  function mutePlayer(playerId) {
    ...
    setVolumePosition(0, playerId);
    $video.$controls.find(selectors.muteIcon).removeClass('hide');
    $video.$controls.find(selectors.unmuteIcon).addClass('hide');
    flash('mute');
  }

  function setVolumePosition(volume, playerId) {
    var $video = videos[playerId] || $activeVideo;
    $video.$controls.find(selectors.volumeBar)[0].value = volume;
    var percent = volume * 100;
    updateVolumeGradient(percent);
  }

  function setVolume(volume, playerId) {
    if (volume > 1) {
      return;
    }
    if (volume < 0) {
    ...
    setVolumePosition(volume, playerId);
  }

  function adjustVolume(delta, playerId) {
    var $video = videos[playerId] || $activeVideo;
    var volume = parseFloat(
      $video.$controls.find(selectors.volumeBar)[0].value
    );
    setVolume(volume + delta);
  }

  function updateVolumeGradient(percent, playerId) {
    var $video = videos[playerId] || $activeVideo;
    var style =
      'background-image: -webkit-gradient(linear, 0% 0%, 100% 0%, color-stop(' +
      percent +
      ...
      percent +
      '%, #A0A0A0));';
    $video.$controls.find(selectors.volumeBar)[0].style = style;
  }

  function playUX(playerId) {
    var $video = videos[playerId] || $activeVideo;
    $video.$controls.find(selectors.playIcon).addClass('hide');
    $video.$controls.find(selectors.pauseIcon).removeClass('hide');
    hidePauseOverlay(playerId);
  }

  function pauseUX(playerId) {
    var $video = videos[playerId] || $activeVideo;
    $video.$controls.find(selectors.playIcon).removeClass('hide');
    $video.$controls.find(selectors.pauseIcon).addClass('hide');
    showPauseOverlay(playerId);
  }
  ...
  function initVideo($video) {
    ...
    videos[$video.data('mediaId')] = {
      mediaId: $video.data('mediaId'),
      player: $video.find(selectors.videoPlayer)[0],
      $controls: $video.find(selectors.controls),
      $volumeWrapper: $video.find(selectors.volumeWrapper),
      $videoWrapper: $video.closest(selectors.videoWrapper),
      $flashOverlay: $video.find(selectors.flashOverlay),
      controlsVisible: false,
      flashVisible: false,
      pauseOverlay: true
    };

    var $productMediaWraps = $video.closest(selectors.productMediaWraps);
    $($productMediaWraps).on('mediaVisible', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
    });
    $($productMediaWraps).on('mediaHidden', function(evt) {
      var playerId = $(evt.currentTarget).data('mediaId');
      pauseVideo(playerId);
    });
  }

  function initControls() {
    $(document).on('click', selectors.playButton, function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      togglePlayback();
    });
    $(document).on('click', selectors.pauseOverlay, function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      togglePlayback();
    });
    $(document).on('click', selectors.muteButton, function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      toggleMute();
    });
    $(document).on('mouseover', selectors.muteButton, function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      showVolumeControl();
    });
    $(document).on('mouseleave', selectors.muteButton, function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      resetVolumeWrapperTimeout();
    });
    $(document).on('input', selectors.seekBar, function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      setTime();
    });
    $(document).on('input', selectors.volumeBar, function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      setVolume(evt.currentTarget.value);
    });

    $(selectors.videoWrapper).on('mousemove', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      showControls();
      resetControlsTimeout();
    });

    $(selectors.controls).on('mousemove', function(evt) {
      evt.stopPropagation();
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      clearTimeout(hideControlsTimer);
    });

    $(selectors.controls).on('mouseleave', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      resetControlsTimeout();
    });

    $(selectors.volumeWrapper).on('mousemove mouseleave', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
  ...
  function initMobileControls() {
    ...
    $(selectors.pauseOverlay).on('touchend', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      togglePlayback();
    });
    $(selectors.videoWrapper).on('touchend', function(evt) {
      evt.preventDefault();
      evt.stopPropagation();
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      showControls();
    });
    $(selectors.playButton).on('touchend', function(evt) {
      evt.preventDefault();
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      togglePlayback();
    });
    $(selectors.muteButton).on('touchend', function(evt) {
      evt.preventDefault();
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      showVolumeControl();
    });
    $(selectors.seekBar).on('touchstart', function(evt) {
      evt.preventDefault();
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      $activeVideo.player.pause();
    });
    $(selectors.seekBar).on('touchmove', function(evt) {
      evt.preventDefault();
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      var percent = rangeTouchPercent(evt);
      setTime(percent);
    });
    $(selectors.seekBar).on('touchend', function(evt) {
      evt.preventDefault();
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      var percent = rangeTouchPercent(evt);
      setTime(percent);
      $activeVideo.player.play();
    });
    $(selectors.volumeBar).on('touchstart touchmove touchend', function(evt) {
      evt.preventDefault();
      resetVolumeWrapperTimeout(750);
    });
    $(selectors.volumeWrapper).on('touchstart', function(evt) {
      evt.preventDefault();
    });
    $(selectors.volumeWrapper).on('touchmove', function(evt) {
      evt.preventDefault();
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      var percent = rangeTouchPercent(evt, true);
      setVolume(percent);
    });
    $(selectors.volumeWrapper).on('touchend', function(evt) {
      evt.preventDefault();
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      var percent = rangeTouchPercent(evt, true);
      setVolume(percent);
  ...
  function initVideoPlayer() {
    $(selectors.videoPlayer).on('click', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      togglePlayback();
    });
    $(selectors.videoPlayer).on('timeupdate', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      updateSeekBar();
    });
    $(selectors.videoPlayer).on('play', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      playUX();
    });
    $(selectors.videoPlayer).on('pause', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      pauseUX();
    });
    $(selectors.videoPlayer).on('ended', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      showPauseOverlay();
    });
  }
  ...
  function initSeekBar() {
    $(selectors.seekBar).on('mousedown', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      $activeVideo.player.pause();
    });
    $(selectors.seekBar).on('mouseup', function(evt) {
      setActiveVideo($(evt.currentTarget).data('mediaId'));
      $activeVideo.player.play();
    });
  }
...
// eslint-disable-next-line no-unused-vars
function onYouTubeIframeAPIReady() {
  theme.ExternalVideo.loadVideos();
}

window.theme = window.theme || {};

theme.ExternalVideo = (function() {
  var selectors = {
    productMediaWraps: '.product-single__media',
    videoWrapper: '.external-video-wrapper'
  };
  ...
  function createPlayer(key) {
    ...
    videoPlayers[key] = new YT.Player(key, { videoId: video.sourceId });
  }
  ...
  function initVideo($video) {
    ...
    videos[$video.data('mediaId')] = {
      mediaId: $video.data('mediaId'),
      sourceId: $video.data('externalSourceId')
    };
...
theme.Product = (function() {
  function Product(container) {
    ...
    this.selectors = {
      ...
      productFeaturedMedia: '.product-featured-media',
    ...
    this.settings.zoomEnabled = $(this.selectors.productMediaWraps).hasClass(
      'js-zoom-enabled'
    );
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
      ...
      enquire.register(this.settings.mediaQuerySmall, {
        match: function() {
          ...
          if (self.settings.zoomEnabled) {
            $(self.selectors.productMediaWraps).each(function() {
              _destroyZoom(this);
            });
          }
    ...
    _initVideos: function() {
      $('.shopify-video-wrapper').each(function() {
        var $el = $(this);
        theme.ShopifyVideo.initVideo($el);
      });
      theme.ShopifyVideo.init();
      $('.external-video-wrapper').each(function() {
        var $el = $(this);
        theme.ExternalVideo.initVideo($el);
      });
    },
    ...
    _setActiveThumbnail: function(mediaId) {
      ...
      if (typeof mediaId === 'undefined') {
        mediaId = $(
          this.selectors.productMediaWraps + ':not(.hide)',
          this.$container
        ).data('media-id');
      }
    ...
    _switchMedia: function(mediaId) {
      var $newMedia = $(
        this.selectors.productMediaWraps + "[data-media-id='" + mediaId + "']",
        this.$container
      );
      var $otherMedia = $(
        this.selectors.productMediaWraps +
          ":not([data-media-id='" +
          mediaId +
          "'])",
        this.$container
      ...
      $newMedia.removeClass(this.classes.hidden);
    ...
    _handleImageFocus: function(evt) {
      ...
      $(this.selectors.productFeaturedMedia + ':visible').focus();
    },
    ...
    _mediaIdFromUrl: function(url) {
      var $newImage = $(
        this.selectors.productMediaWraps +
          " .product-featured-media[data-match-url='//" +
          url +
          "']",
        this.$container
      );

      var $newMedia = $newImage.closest(this.selectors.productMediaWraps);
      return $newMedia.data('mediaId');
    },
    ...
    _updateImages: function(evt) {
      ...
      var mediaId = this._mediaIdFromUrl(imageUrl);
